...
document.addEventListener("DOMContentLoaded", function(event) { 
	...
	window.addEventListener('resize', fixButtonWidth);
	...
	function buildTimeline(progress) {
		...
		tl.fromTo('.card:nth-child('+(cards.length)+')', {yPercent:0}, {yPercent:-100, duration: d, ease: 'linear'});
		tl.fromTo('.card:nth-child('+(cards.length)+') .container', {yPercent:0}, {yPercent:100, duration: d, ease: 'linear'},"-="+d);
		...
		tl.fromTo('.card:nth-child('+(cards.length+1)+')', {yPercent:0}, {yPercent:100, duration: d, ease: 'linear'},"-="+d);
		tl.fromTo('.card:nth-child('+(cards.length+1)+') .container', {yPercent:0}, {yPercent:-100, duration: d, ease: 'linear'}, "-="+d);
	...
	function closeMenu() {
		...
		setTimeout(() => mainMenu.classList.add('hidden'), 600);
	}
